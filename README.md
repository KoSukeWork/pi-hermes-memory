...
| `childExtensionPaths` | unset | Trusted provider/auth extension sources explicitly allowed in isolated child Pi processes. Allowed values: existing local files (`~/...` and relative paths are expanded), `npm:` specs, `git:github.com/...` / `git:git@github.com:...`, and `https://github.com/...`. Control characters, `..` segments, and other URL schemes are rejected. Only `pi-claude-auth` and `@gotgenes/pi-anthropic-auth` are auto-detected from sibling packages; other adapters must be listed here. In-process direct transport (the default for review/flush/correction/consolidation) usually does not need it, since it reads whatever provider auth is already registered. |
