...
register("real bootstrap declares matching capabilities and loads the real factory", { timeout: 120000 }, async () => {
	...
	await bootstrap(pi);
	for (const name of ["memory-insights"]) {
	...
	const commandIdentity = new Map(["memory-insights"].map((name) => [name, commands.get(name)]));
	await emit("tool_call");
	const realigned = ["memory-insights"].some((name) => commands.get(name) !== commandIdentity.get(name));
	assert.ok(
